...
bool Fleet::isHitNSink(const Location& userShot) {
	...
	for (int numOfDeployed = 0; numOfDeployed < 6; ++numOfDeployed) {

		if (ships_[numOfDeployed].match(userShot) == true) {
			cout << "Hit! " << endl;
			ships_[numOfDeployed].sink();
			++numOfDeployed;
			return true;

			/*if (ships_[numOfDeployed].isSunk() == true)
				cout << "This ship has already been sunk. " << endl;*/
		}
	}
	cout << "Miss " << endl;
}
